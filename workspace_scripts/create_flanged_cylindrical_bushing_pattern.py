...
from cad_runtime.solidworks.patterns import circular_feature_pattern
from cad_runtime.solidworks.patterns import iter_features
...
def seed_circle_cut(model, plane_name: str, sketch_name: str, feature_name: str, x_mm: float, y_mm: float, diameter_mm: float):
    begin_sketch(model, plane_name, sketch_name)
    draw_circle(model, x_mm, y_mm, diameter_mm / 2)
    finish_sketch(model, sketch_name)
    feature = extrude_cut_through_all(model, feature_name, preferred_reverse_direction=True)
    log(f"Seed cut created: {feature_name}, diameter {diameter_mm} mm")
    view_update(model)
    return feature


def feature_names(model) -> set[str]:
    names = set()
...
def create_center_axis_from_default_planes(model) -> str:
    before = feature_names(model)
    clear_selection(model)
    select_plane(model, "Front Plane", append=False)
    select_plane(model, "Right Plane", append=True)
    result = model.InsertAxis2(True)
    log(f"InsertAxis2 center axis result: {result}")
    rebuild_model(model)
    axis_candidates = []
    for feature in iter_features(model):
        try:
        ...
        if name in before:
            continue
        if name.startswith("基准轴") or name.startswith("Axis"):
            axis_candidates.append(feature)
    if not axis_candidates:
        raise RuntimeError("Center axis was not found after InsertAxis2.")
    axis = axis_candidates[-1]
    axis.Name = "AXIS_CENTER_BORE"
    log("Datum axis created: AXIS_CENTER_BORE")
    view_update(model)
    return "AXIS_CENTER_BORE"
...
def save_exports(model) -> list[Path]:
    ...
    paths = [
        EXPORT_DIR / "flanged_cylindrical_bushing_pattern_v2.SLDPRT",
        EXPORT_DIR / "flanged_cylindrical_bushing_pattern_v2.STEP",
        EXPORT_DIR / "flanged_cylindrical_bushing_pattern_v2.x_t",
    ]
    for path in paths:
...
def main() -> int:
    ...
    create_center_axis_from_default_planes(model)
    ...
    seed_circle_cut(
        model,
        "Top Plane",
        "SK_BOLT_HOLE_SEED_D8_PCD62",
        "BOLT_HOLE_SEED_D8_THROUGH_FLANGE",
        bolt_radius,
        0,
        bolt_diameter,
    )
    log("Creating native circular pattern for flange bolt holes")
    circular_feature_pattern(
        model,
        "BOLT_HOLE_CIRCULAR_PATTERN_4X_D8_PCD62",
        "BOLT_HOLE_SEED_D8_THROUGH_FLANGE",
        "AXIS_CENTER_BORE",
        int(bolt_count),
        total_angle_deg=360,
    ...
    log("Native circular pattern created: BOLT_HOLE_CIRCULAR_PATTERN_4X_D8_PCD62")
    view_update(model)
    ...
    result = {
        "status": "completed",
        "pattern_strategy": "one seed bolt hole plus native SolidWorks circular feature pattern",
